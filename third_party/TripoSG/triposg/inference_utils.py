...
# ProtoSkin patch: `diso` (DiffDMC) is CC BY-NC 4.0, not open source, and is
# only used by flash_extract_geometry. Import it lazily there so the package
# imports without it; ProtoSkin always uses use_flash_decoder=False, i.e.
# hierarchical_extract_geometry (scikit-image marching cubes, BSD).
...
def flash_extract_geometry(
    ...
    **kwargs,
    ...
    try:
        print("final grids shape = ", grid_logits.shape)
        from diso import DiffDMC  # ProtoSkin patch: lazy, see top of file
